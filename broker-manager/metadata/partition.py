...
class Partition(Service):
    ...
    def get_gobal_topic_partition(self, topic_id):
        return json.loads(self.db.hget("topic_partition", topic_id).decode("utf-8"))
    
    def set_gobal_topic_partition(self, topic_id, partitions):
        self.db.hset("topic_partition",topic_id,json.dumps(partitions))
    
    def add_gobal_topic_partition(self, topic, partition):
        # Get topic_partition
        topic_partition = self.get_gobal_topic_partition(topic)
        ...
        self.set_gobal_topic_partition(topic,topic_partition)